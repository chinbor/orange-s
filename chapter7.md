...
* **主要看tasksys函数和TestA函数，因为只有这两个进程在进行消息传递，对于接受方进程，主要看 msg\_receive 函数，对于发送方进程，主要看 msg\_send 。还有注意死锁的发生也要看一下，代码注释中有。还有就是消息拷贝也需要注意下。当然对于一个进程接受多个发送方的消息，需要一个 q\_sending 成员用来记录。结合代码看一下书中320页的消息发送以及接受过程**
