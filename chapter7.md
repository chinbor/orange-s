...
> 使用了很多指针的程序设计，和内存地址紧密相连，对于int \*p，\*p占用4个字节，那么p++就是地址加4，因为int4字节，char \*p，由于\*p占用1个字节，所以p++就是地址加1

## orange's 第八章学习笔记

* **主要看tasksys函数和TestA函数，因为只有这两个进程在进行消息传递，对于接受方进程，主要看 msg\_receive 函数，对于发送方进程，主要看 msg\_send 。还有注意死锁的发生也要看一下，代码注释中有。还有就是消息拷贝也需要注意下。当然对于一个进程接受多个发送方的消息，需要一个 q\_sending 成员用来记录。结合代码看一下书中320页的消息发送以及接受过程，哦对了，还有就是vsprintf函数中有一个地方有问题，我在代码中注释了的。反正详细的见代码吧。**

[&lt; prev](chapter6.md) [next &gt;](chapter8.md)
