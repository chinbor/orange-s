...
> ![](/assets/import.png)
>
> #### 段基地址仍是32位，其值是相应段寄存器值\(段值\)乘以16，在把段值装载到段寄存器时刷新。由于其值是16位段值乘上16，所以在实模式下基地址实际上有效位只有20位。每个段的32位段界限都固定为0FFFFH，段属性的许多位也是固定的。所谓固定是指在实方式下不可设置这些属性值，只能继续沿用保护方式下所设置的值。因此，在准备结束保护模式回到实模式之前，**要通过加载一个合适的描述符选择子（如实例代码中的Normal选择子）到有关段寄存器，以使得对应段描述符高速缓冲寄存器中含有合适的段界限和属性。**必须在保护模式下设置好段高速缓冲寄存器的值，因为一旦到了实模式下就不能在改变了。
