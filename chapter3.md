...
> ##### 代表的意思就是32字节对齐，也地址就是能够整除32，因为在bits 32模式下，所有的操作码，操作数，地址都是32位，也就是4字节，所以采用32字节对齐，相当于能够有8个4字节，基本能够存放一般的指令码，同时可以加快地址的访问速度，以及内存的合理分配/
...
> 1.页目录表和页表都存在内存中
>
> 2.分页首先需要初始化页目录表的内容和页表的内容，同时注意低12位的设置（一些关于页的属性），页目录表中设置的低12位影响的是一组物理页的权限，页表中每一项的低12位影响的是一个物理页的权限，所以需要组合这两个值，具体组合值google，还有需要注意的是每一个页4kb
>
> 3.只要不特别的设置映射关系，按照orange‘s书中的代码，那么线性地址都是等于物理地址的，公式为：（线性地址的高10位\*1024\*4096）+（线性地址中间10位\*4096）+低12位。注意都用的10进制
>
> 4.在采取了分页的存储管理中，程序都是用的线性地址（很大），不用关心物理内存

[< prev](chapter2.md) [ next >](chapter4.md)
